...
void CircularLinkedList::executeProcesses(int quantum) 
{
    ...
    while (head != NULL) 
    {
        cout << "\nExecuting Process P" << curr->id << " (Remaining Burst Time: " << curr->burst_time << ")\n";

        if (curr->burst_time > quantum) 
        {
            curr->burst_time -= quantum;
            cout << "Process P" << curr->id << " executed for " << quantum << " units, Remaining: " << curr->burst_time << "\n";
            prev = curr;
            curr = curr->next;
        } 
        else 
        {
            cout << "Process P" << curr->id << " executed for " << curr->burst_time << " units and completed.\n";
